...
|---|---|---|
| C1 | `MY_ENTRY.md` and `MY_ENTRY/-this.md` are "the same" — undefined if both exist | Folder form wins, plain file is shadowed and reported by the DB check. Adding a child auto-promotes `X.md` -> `X/-this.md`; removing the last child does **not** demote back, the folder stays |
...
| C8 | No ordering rule, yet the list has "load more" | One deterministic sort from the front matter (`created`, newest first), paged by offset |
...
| C10 | Delete semantics for linked / linking objects missing | Hard delete after a confirm that names the object and, for a container, its child count. Delete link = target untouched. Delete target = referring links listed in the confirm, then they become broken-link cards |
...
**No `title` key** — the file or folder name is the title, one source of truth. Three field names are mapped, everything else is a plain front matter key:

| Field in the type config | Stored in |
|---|---|
| `title` | the file / folder name (editing it renames, see §2.8) |
| `body` | the markdown body below the front matter |
| anything else | a front matter key |

The DB root itself has no `-this.md` and is treated as an implicit `Group` named after the folder.
...
```json
{
  "builtAt": 1754563200,
  "objects": {
    "workshop-2026-08-07-1030": { "path": "Kickoff process/02 Workshop", "type": "Activity", "mtime": 1754563100 }
  }
}
```
...
- Duplicate id (copied file): first one found wins for link resolution, both are listed by `Db::check()` and marked on the card, so the user can fix it. The app never silently rewrites an id.
...
The title field in the editor **is** the file name — editing it renames the file or folder. Nothing is stored twice, so nothing can drift.
...
file name             =  the input verbatim, spaces kept, only forbidden chars replaced
...
manual file rename    -> id unchanged, next index refresh picks up the new path,
                         and the new name is simply the new title
```

- Renaming a link file changes only its label — the target is untouched.
- A rename is the one edit that is **not** autosaved on every keystroke: it commits on blur or Enter (§4.4), so typing does not produce a trail of renamed files.

### 2.9 Create

```
[+] -> type chosen  ->  file name = "New <Type>"   ("New Activity", "New Activity 2", ...)
                        id        = from idPattern
                        created   = now
                        object opens in the editor, title field focused and selected
```

Because the title is the file name, a new object must have a name before it exists — the generated one is a placeholder the user overwrites immediately.

### 2.10 Delete

**Hard delete**, no trash, no undo — the DB is plain files, so git or a backup is the safety net.

```
confirm dialog names       leaf       -> "Delete 'Agenda'?"
                           container  -> "Delete 'Workshop' and its 7 entries?"
                           linked to  -> + "3 links point here and will break"

then  unlink / rmdir -r   ->  index entry removed  ->  list refreshes
                              referring links stay and render as broken-link cards
```
...
fields:                                   # hierarchical
  title:  { type: 'text',     required: true }   # mapped to the file name
  status: { type: 'select',   options: ['open', 'running', 'done'] }
  body:   { type: 'markdown', label: 'Description' }   # mapped to the markdown body
  schedule:
    fields:
      start: { type: 'date' }
      end:   { type: 'date' }
...
# include:  inline = objects lying in the folder, link = link stubs.
#           A link is matched by its *target's* type, and the order of the
#           values is the order the two groups appear in the list.

create:                                   # [+] dropdown, per tab
  entries:      ['Activity', 'Info']      # new object of that type
  participants: ['__link:Person']         # picker dialog, creates a link stub
...
detailTabs:                               # optional tabs *inside* the editor, omit for a single pane
  ...
  - { id: 'raw',      label: 'Front matter' }   # id read-only, type switchable, see §5.6
...
| `File` | resource handler | — | no stored instances, handles `NAME.ext`. Editor: preview for images / pdf / text, otherwise icon + metadata, always an open/download link; sidecar fields editable |
...
composer.json         symfony/yaml + PSR-4 autoload for /lib
config.yml            app config (dbPath, theme, pageSize override, ...)
controller.js         app controller
file.php              serves a resource file out of the database
index.php             single page shell
```

- Icons: one SVG sprite in `/styles/icons.svg`, referenced by the `icon` key of a type. No icon font, no external requests.

### 4.1 Core classes (`/lib`)

| Class | Responsibility |
|---|---|
| `App` | boots and wires the services, one instance per request |
| `Config` | app `config.yml` + db `.sys/config.yml` |
| `Db` | database root, path safety, health check |
| `ObjPath` | path <-> object rules (`-this`, `(lnk)` marker, folder vs file, promotion, name sanitizing) |
...
| `View` | renders a view file to a string, so first paint and ajax share the templates |
| `FileSize` | human readable sizes for the `File` renderers |
| `Failure` | error carrying the http status the ajax layer answers with |
...
| `list` | `path, tab, offset, filter` | rendered cards + `hasMore`, `total` |
...
| `tree` | `path, depth, types` | hierarchy fragment — one dialog serves **move target** and **link target**, `types` narrows it to what the tab accepts |
...
blur field ---------------------->|--> save  --> card in the list re-rendered
leave editor / navigate --------->|            --> footer state updated
close AI, fold column, mobile back'

title field: not debounced, commits on blur or Enter (it renames a file)
```

| Situation | Behaviour |
|---|---|
| Save in flight, user types on | Queue one follow-up save; never two writes in parallel for the same object |
| `409` stale mtime | Editor keeps the user's text, footer turns into a warning with "reload" / "overwrite" |
| Validation fails (`required`) | Field marked, nothing written, footer shows the reason |
| Object deleted elsewhere | Next save returns `404` -> editor closes with a note, list refreshes |
| Navigating away with a pending debounce | Flushed immediately before the navigation completes |

Save state is app level and rendered at the **right end** of `FOO_2` ("saved 12:04", "saving...", "unsaved"); the type's `footer_renderer` fills the left part.
...
+------------------+                           +----------------------+
| [filter]         |   detail editor           |                      |  36px  tools (tab Entries)
...
| Footer | list state, e.g. "12 entries" | `FOO_2` from `footer_renderer`, spanning mid **and** AI column. Left part: type content. Right end: app level save state (§4.4) |
...
- Dummy behaviour in v1: the composer is **live**, appends the typed message to the transcript and answers with a fixed "not connected yet" — so the layout can be judged with real content. `+` and `v` are inert.
...
| 900 .. 1199 | nav \| mid | content area splits and the **nav column steps aside** while the AI column is open (built: it hides completely, an icon rail carrying only a back arrow would not earn its width) |
...
```
container = object whose children the left list shows   (the DB root is an implicit Group)
selection = object opened in the mid column
depth     = relative to the container: its children are depth 1, their children depth 2

click on card
  |- leaf                 -> select, render in mid column
  |- container at depth 1 -> unfold its children inline AND select it
  |- its chevron          -> unfold / collapse only, no selection
  |- container at depth 2 -> drill in: it becomes the container, "<" appears
  |- link                 -> jump to the target's real location, then select
[edit] in left header     -> select the container itself ("Current")
"<"    in left header     -> back to the parent container (hidden at root)
```

A container is an entry with content of its own, so clicking it opens the
editor like any other entry; the chevron is there for pure unfolding.
...
Visible on the `entries` tab only (`tools: true` in the tab config).

```
[ filter ... ]        text filter on the title, matches while typing

already loaded pages  -> filtered in JS, instant
beyond the loaded set -> ajax list&filter=..., paging continues filtered
empty input           -> back to the plain list, offset reset
```

Sorting has no control in v1 — there is exactly one sort (§2.7).

### 5.6 Front matter tab

```
id:    workshop-2026-08-07-1030   [copy]    read-only, editing it would break every
                                            incoming link silently
type:  [ Activity        v ]                switchable, editor re-renders with the
                                            new type's fields
owner: anna                                 unknown keys: editable, kept verbatim
[+ add key]
```

- Switching the type keeps every key; fields the new type does not declare stay as unknown keys instead of being dropped.
...
- **Move** opens a dialog with the hierarchy tree (`tree` action, lazy per level), target picked by click. No drag & drop in v1.
...
| M4 | Detail renderer, header/footer renderers, `...` menu, autosave (§4.4), create/delete, rename, mtime guard, move dialog | full CRUD on Activity and Info |
...
| M8 | AI sidebar dummy (header, session dropdown, live composer with a canned reply, no backend) | UI complete |
