...
| C12 | Unknown / missing `type:` | Falls back to `defaultType` (`Info`); the raw front matter stays editable and is never lost on save |
...
| M2 | TypeRegistry, Type, Renderer base, `Activity`/`Info` list renderers, `index.php` shell, left column | list renders from the real DB |
